...
class ControlStrategy(Node):
    ...
    def inter_direction_diff_drive(self, ):
        if(self.robot_current_pose is not None):
            ...
            self.D = np.sqrt((self.robot_current_pose[0]-self.refPose[0])**2 
            ...
            phiR = np.arctan2(self.refPose[1]-self.robot_current_pose[1], self.refPose[0]-self.robot_current_pose[0])
            ...
            if(np.abs(alpha) < np.abs(beta)):
                ePhi = self.wrap_to_pi(phiR - self.robot_current_pose[2] + alpha) 
            else:
                ePhi = self.wrap_to_pi(phiR - self.robot_current_pose[2] + beta) 
                
            v = self.k_p*self.D
            w = self.k_w*ePhi
            print("Distance to the goal: ", self.D)
            dq = np.array([v*np.cos(self.robot_current_pose[2]+self.Ts*w/2)
                            , v*np.sin(self.robot_current_pose[2]+self.Ts*w/2), w])
    ...
    def inter_point_diff_drive(self, ):
        if(self.robot_current_pose is not None):
            ...
            if self.state == 0:
                d = np.sqrt((self.yT-self.robot_current_pose[1])**2 
                            + (self.xT-self.robot_current_pose[0])**2)
                if(d < self.dmin):
                    self.state = 1
                self.phiT = np.arctan2(self.yT-self.robot_current_pose[1], self.xT-self.robot_current_pose[0])
