...
class ControlStrategy(Node):
    def __init__(self, delta_t, ):
        ...
        self.t = np.arange(0, 60, self.Ts) # Simulation time
    ...
    def perform_action_diff_drive_one_step(self):
        if(self.robot_current_pose is not None):
            ...
            v = self.r/2*(self.wR+self.wL) # velocity
            w = self.r/self.L*(self.wR-self.wL) # angular
            #TODO Estimate robot dynamics, the robot current pose (x_{t}) can be obtained by self.robot_current_pose
            # dq = self.robot_current_pose
            dq = np.array([
...
def main(args=None):
    ...
    control_strategy.diff_drive_init(10, 10)
    while rclpy.ok():
        try:
