...
class ControlStrategy(Node):
    ...
    def inter_point_diff_drive_init(
        self,
        duration=10,
        r_distance=0.5,
        refPose=np.array([1, 1, 0]),
        k_p=0.5,
        k_w=0.7,
        dmin=0.7,
    ):
    ...
    def inter_direction_diff_drive_init(
        self,
        duration=10,
        r_distance=1.3,
        refPose=np.array([1.5, 1.5, np.pi / 2]),
        k_p=0.5,
        k_w=0.7,
        dmin=0.7,
    ):
    ...
    def inter_direction_diff_drive(
        self,
    ):
        if self.robot_current_pose is not None:
            ...
            self.D = np.sqrt(
                ((self.robot_current_pose[0] - self.refPose[0]) ** 2)
                + ((self.robot_current_pose[1] - self.refPose[1]) ** 2)
            )
            ###
            ...
            phiR = np.arctan2(
                self.refPose[1] - self.robot_current_pose[1],
                self.refPose[0] - self.robot_current_control[0],
            )
            ...
            dq = np.array(
                [
                    v * np.cos(self.robot_current_pose_real[2] + self.Ts * w / 2),
                    v * np.sin(self.robot_current_pose_real[2] + self.Ts * w / 2),
                    w,
                ]
            )
            ...
            self.robot_current_pose[2] = self.wrap_to_pi(
                self.robot_current_pose[2]
            )  # Map orientation angle to [-pi, pi]
            self.send_vel(v, w)
            self.time_utilized = self.time_utilized + self.Ts

    def inter_point_diff_drive(
        self,
    ):
        if self.robot_current_pose is not None:
            if self.duration < self.time_utilized:
            ...
            # TODO Estimate orientation residual (self.ePhi)
            if self.state == 0:
                # Check whether the robot has already reached a valid distance from the goal position.
                d = np.sqrt(
                    (self.yT - self.robot_current_pose[1]) ** 2
                    + (self.xT - self.robot_current_pose[0]) ** 2
                )
                if self.dmin > d:
                    # If so, change the state to 1 to change the residual logic.
                    self.state = 1
                self.phiT = np.arctan2(
                    self.yT - self.robot_current_pose[1],
                    self.xT - self.robot_current_pose[0],
                )
                self.ePhi = self.phiT - self.robot_current_pose[2]
            else:
                self.ePhi = self.refPose[2] - self.robot_current_pose[2]
            ###
    ...
    def perform_action_diff_drive_one_step(self):
        if self.robot_current_pose is not None:
            ...
            dq = np.array(
                [
                    v * np.cos(self.robot_current_pose[2] + self.Ts * w / 2),
                    v * np.sin(self.robot_current_pose[2] + self.Ts * w / 2),
                    w,
                ]
            )
            self.robot_current_pose = (
    ...
    def set_pose(self, msg):
        ...
        if self.set_robot_init_pose is None:
            self.set_robot_init_pose = np.array(
                [msg.pose.pose.position.x, msg.pose.pose.position.y, yaw]
            )
            self.robot_current_pose = self.set_robot_init_pose
        self.robot_current_pose_real = np.array(
            [msg.pose.pose.position.x, msg.pose.pose.position.y, yaw]
        )
